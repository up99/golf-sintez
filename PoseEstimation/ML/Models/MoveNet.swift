...
final class MoveNet: PoseEstimator {
  ...
  func estimateSinglePose(on pixelBuffer: CVPixelBuffer) throws -> (Person, Times) {
    ...
    postprocessingTime = Date().timeIntervalSince(postprocessingStartTime)
      ...
      // CHAT
    // ТУТ ХРАНИТСЯ ИНФОРМАЦИЯ О КООРДИНАТАХ
//      print(result.keyPoints)
      
      // Assuming you have the array of PoseEstimation.KeyPoint objects
      let keyPoints: [PoseEstimation.KeyPoint] = result.keyPoints

      for keyPoint in keyPoints {
          let bodyPart = keyPoint.bodyPart
          let coordinate = keyPoint.coordinate
          let score = keyPoint.score
          
          // Now you can use bodyPart, coordinate, and score as needed
          print("Body Part: \(bodyPart)")
          print("Coordinate: \(coordinate)")
          print("Score: \(score)")
          
          // If you want to access the individual components of the coordinate (x, y):
          let x = coordinate.x
          let y = coordinate.y
